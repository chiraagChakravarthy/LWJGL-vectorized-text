...
public class GlyphRenderer5 {
    ...
    int scale = 4;//number of screen pixels per buffer pixel
    ...
    public void render(Graphics2D g) {
        ...
        for (int x = 0; x < image.getWidth(); x++) {
            for (int y = 0; y < image.getHeight(); y++) {
                ...
                float qx = (float) ((x-0.1f*width/scale)*zoom-ticks*Math.PI);
                float qy = (float) ((y-0.1f*height/scale)*zoom-ticks*Math.PI);
    ...
    private float area(float x, float y, int glyph) {
        ...
        for (float[] bezier : beziers) {
            ...
            //at^2+bt+c-x=0
            {
                float a = bezier[0], b = bezier[1], c = bezier[2];
                findRoots(a, b, c-xMin, roots, 0);
                findRoots(a, b, c-xMax, roots, 2);

                a = bezier[3];
                b = bezier[4];
                c = bezier[5];
                findRoots(a, b, c-yMin, roots, 4);
                findRoots(a, b, c-yMax, roots, 6);
                roots[8] = 0;
                roots[9] = 1;
            }
            ...
            * enter/exit square
            ...
            for (int i = 0; i < roots.length-1; i++) {
                ...
                if(side==2||side==3) {
                    ...
                    if((ix<xMin+epsilon&&dx<0)||ix<xMin||(ix>xMax-epsilon&&dx>0)||ix>xMax){
                        bounds = false;
                    }
                ...
                if(side==0||side==1){
                    ...
                    if((iy < yMin+epsilon&&dy<0)||iy<yMin){
                        bounds = false;
                    }
                    if((iy>yMax-epsilon&&dy>0)||iy > yMax){
                        clamp = true;
                    }
                }
                if(side==4){
                ...
                if(bounds){
                    ...
                    if(clamp){
                        if(Math.abs(nextIx-ix)>zoom){
                            int k = 0;
                        }
                        overlap += (nextIx-ix)*zoom;
                    } else {
                        overlap += integrate(bezier, yMin, t, t1);
                    }
                }
            }
        }
    ...
    */
    private float integrate(float[] bezier, float y, float t0, float t1){
        float a = bezier[0];
        float b = bezier[1];
        float d = bezier[3];
        float e = bezier[4];
        ...
        //(x(b(6F+x(3E+2Dx))+ax(6F+x(4E+3Dx))))/6
        float upper = (t1*(b*(6*f+t1*(3*e+2*d*t1))+a*t1*(6*f+t1*(4*e+3*d*t1))))/6f;
        float lower = (t0*(b*(6*f+t0*(3*e+2*d*t0))+a*t0*(6*f+t0*(4*e+3*d*t0))))/6f;
