...
public class TextRenderer2 {
    ...
    private static int shader, u_Atlas, u_EmScale, u_Mvp, u_Viewport, u_FontLen;
    ...
    public TextRenderer2(VectorFont font){
        this.font = font;
    }

    public TextRenderer2(){
        this(new VectorFont("/font/arial.ttf"));
    }
    ...
    private static void initShaders() {
        shader = createShader("/shader/text2.vert", "/shader/text2.frag");
     ...
     */
    public void drawText(String text, Matrix4f pose, Vector2f align, TextBoundType alignType, Vector4f color){
        if(text.isEmpty()){
            return;
        }
        assertInitialized();
        ...
        for(int i = this.len; i < MAX_LEN && i < this.len+text.length(); i++){
            Matrix4f letterPose = new Matrix4f(pose).mul(new Matrix4f().translate(advance[i-this.len], 0, 0));
            appendToInstanceBuffer(letterPose, font.indexOf(characters[i-this.len]), color, i);
        }
    ...
    public void render(Matrix4f mvp){
        ...
        glUniformMatrix4fv(u_Mvp, false, mvp.get(matrixArray));
