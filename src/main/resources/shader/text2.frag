...
//i3, i2, i1, k4, k3, k2, k1, k0
vec2 computeWindow(float A, float B, float C, float D, float E, float F, float G, float H, vec2 ta, vec2 tb, float R2){
    float invR2 = 1/R2;
    float invR4 = invR2*invR2;

    vec2 tb2 = tb *tb, ta2 = ta *ta;
    vec2 tb3 = tb2*tb, ta3 = ta2*ta;
    vec2 tb4 = tb3*tb, ta4 = ta3*ta;
    vec2 tb5 = tb4*tb, ta5 = ta4*ta;
    vec2 tb6 = tb5*tb, ta6 = ta5*ta;
    vec2 tb7 = tb6*tb, ta7 = ta6*ta;
    ...
    float i7 = A*D/7,
            ...
            i2 = (G*C + H*B)/2,
            i1 = H*C;
    vec2 integral = 2*invR2*(A/3*(tb3-ta3)+B/2*(tb2-ta2)+C*(tb-ta));
    //integral -= invR4*(i7*(tb7-ta7) + i6*(tb6-ta6) + i5*(tb5-ta5) + i4*(tb4-ta4) + i3*(tb3-ta3) + i2*(tb2-ta2) + i1*(tb-ta));
    return integral;
}
...
float calcArea(vec2 pos, float r){
    ...
    for (int i = start; i < end; i++) {
        ...
        vec4 roots = clamp(solveQuartic(k4, k3, k2, k1, k0-R2), vec4(0), vec4(1));
        ...
        vec2 window = computeWindow(i3, i2, i1, k4, k3, k2, k1, k0, roots.xz, roots.yw, R2);
        delta -= window.x + window.y;

        delta -= angleIntegrate(a, b, c, d, e, f, 0, roots.x);
        delta -= angleIntegrate(a, b, c, d, e, f, roots.y, roots.z);
        delta -= angleIntegrate(a, b, c, d, e, f, roots.w, 1);
        total += delta;
    }
